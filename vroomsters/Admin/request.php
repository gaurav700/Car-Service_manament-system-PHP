...
<div class="col-sm-4 mb-5">
 ...
 if($result->num_rows > 0){
  while($row = $result->fetch_assoc()){
   ...
   echo '<form action="" method="POST"> <input type="hidden" name="id" value='. $row["request_id"] .'><input type="submit" class="btn btn-success mr-3" name="view" value="View"><input type="submit" class="btn btn-secondary" name="close" value="Close"></form>';
   echo '</div>' ;
   echo '</div>' ;
   echo'</div>';
  }
